...
- (void) drawRect:(CGRect)rect
{
    ...
    for(NSUInteger i = 0; i < numPlots; i++) {
        ...
        DPGraphType plotType = [self.dataSource graphView:self typeOfPlotForIndex:i];
        ...
        if (plotType == DPGraphTypeContinuous) {
            [self drawContinuousPlotWithIndex:i];
...
- (void) drawContinuousPlotWithIndex:(NSUInteger)plotIndex
{
    UIBezierPath *path = [[UIBezierPath alloc] init];
...
- (void) drawDiscretePlotWithIndex:(NSUInteger)plotIndex
{
    UIBezierPath *path = [[UIBezierPath alloc] init];
    
    NSInteger numPoints = [self.dataSource graphView:self numberOfPointsForIndex:plotIndex];
    
    if (numPoints < 2) {
        return;
    }
    
    CGPoint firstPoint = [self.dataSource graphView:self xyValueForPointIndex:0 forPlotIndex:plotIndex];
    firstPoint = [self viewSpaceCoordinateForCartesianCoordinate:firstPoint];
    [path moveToPoint:firstPoint];
    
    for(NSUInteger i = 1; i < numPoints; i++) {
        CGPoint point = [self.dataSource graphView:self xyValueForPointIndex:i forPlotIndex:plotIndex];
        point = [self viewSpaceCoordinateForCartesianCoordinate:point];
...
- (void) drawHistogramWithIndex:(NSUInteger)plotIndex lineHistogram:(BOOL)lineHistogram
{
    UIBezierPath *path = [[UIBezierPath alloc] init];
    ...
    for(NSUInteger i = 0; i < self.numberOfHistogramBars; i++) {
        ...
        if (lineHistogram) {
            ...
            CGPoint topLeft = [self viewSpaceCoordinateForCartesianCoordinate:CGPointMake(start, height)];
            UIBezierPath *bar = [UIBezierPath bezierPathWithRect:CGRectMake(topLeft.x, topLeft.y, [self viewSpaceWidthForCatesianWidth:chunkSize], [self viewSpaceHeightForCatesianHeight:height])];
